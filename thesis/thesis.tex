...
The central design choice of the Shard distributed system was creating a new programming language, rather than writing a library or a set of macros for an existing programming language.
...
The defining feature of the Shard programming language that separates it from the POSIX shell is the it provides the ability for programs to execute commands on a remote machine.
...
The remote command execution is facilitated by using the Shard protocol to facilitate the execution of the remote command, designed specifically to work with the Shard shell, and focusing on the core pillars of being distributed and being reliable.
The Shard protocol works by using a pair of communication channels between the local machine where the Shard program originated, and the remote machine where the Shard command should be executed, with messages being passed along the channel to perform the executing of the Shard command on the remote machine, and to ensure this execution is performed in a reliable manner.
A custom protocol is used because it affords Shard full control over ensuring reliability, with a combination of features such as heartbeats, sequence numbers, and retry mechanisms to maximize the reliability of .
The Shard protocol also makes it possible to create

% segue into application classes
\todoi{Close out this section}
...
The Shard programming language is designed primarily to satisfy the core pillar of being easy to use.

\todoi{Write more in this section}

The syntax of the new features of the Shard programming language are shown in Figure \ref{fig:pl_syntax}, and the parameters for the syntax is shown in \ref{fig:pl_syntax_parameters}.
  ...
  \begin{center}
    \begin{tabular}{|l|l|l|}
      ...
      \multirow{4}{*}{Remote execution} & (PROGRAM)@@TARGET         & (cat file.txt)@@example              \\ \cline{2-3}
                                        & PROGRAM@@TARGET           & hostname@@127.0.0.1                  \\ \cline{2-3}
...
\subsubsection{Single command}
\begin{sloppypar}
  The first experiment in this section, \texttt{hostname.shard}, involves using verifying the \texttt{single\_command} application class works correctly on a simple program.
\end{sloppypar}
\begin{lstlisting}[language=Shard]
# hostname.shard
source cluster_setup.shard
hostname@@testcluster
\end{lstlisting}
This Shard program begins by including a \texttt{cluster\_setup.shard} script to set up the cluster \texttt{testcluster}.
Then, the program calls the command \texttt{hostname} on each of the machines in the cluster \texttt{testcluster}, and prints out the result to the local machine's standard output.
...
The \texttt{hostname.shard} program is ran a total of 100 trials sequentially on each of the three clusters.
Figure \ref{fig:hostname} shows the cumulative number of trials complete at a given point in time.
...
\begin{figure}[h]
  \begin{center}
    ...
    \vspace{2cm}
    \caption{Cumulative number of trials complete given a certain amount of time passed for the \texttt{hostname.shard} experiment}
    \label{fig:hostname}
  \end{center}
\end{figure}
...
This passage has a total of PLACEHOLDER words, which means that each remote machine would have to execute multiple tasks, as opposed to a single task as is the case for the \texttt{hostname.shard} experiment.
The full text of the input document and the expected output can be found in the appendix.
\todoi{Evaluate whether this is a good choice for input text - maybe use this input text for the ``Reliable'' section instead; also figure out copyrights; and add the input/output to the appendix}
...
\subsubsection{Data parallel with delay}
...
For more information on the error handling features of Shard, see the \textbf{Shard protocol} section in the \textbf{Design} chapter.
...
The full text of the input document and the expected output can be found in the appendix.
\todoi{Add the input/output to the appendix}

\subsubsection{Unreachable host}
The first experiment in this section tests the behavior of Shard in the case that a host cannot be reached.
This is done by adding an additional host to the \texttt{cluster\_setup.shard} script, such as \texttt{example.com}, that is known to be unreachable.
...
\todoi{Actually run this experiment to verify the above; What graph/figure should I include for the above case?}
...
\subsubsection{Remote crashes}
The second experiment in this section tests the behavior of Shard in the case that the remote Shard program crashes.

\todoi{Experiment 2: \texttt{kill.shard} (Randomly kill the remote Shard)}

\subsubsection{Packet loss}
The third experiment in this section tests the behavior of Shard in the case that packets are randomly lost.

\todoi{Experiment 3: \texttt{packet\_loss.shard} (Drop certain percentage of returning packets)}

\subsubsection{Packet loss}
The fourth experiment in this section tests the behavior of Shard in the case that certain machines are consistently very slow to complete their tasks.

\todoi{Experiment 4: \texttt{timeout.shard} (Randomize slowdown, with some machines taking very long)}

\todoi{Compare to hacked-up version of other shell?}
...
\todoi{Several example single application/data parallel programs with comparison: number of characters, subjective comparison; also comparison to existing shell languages}
