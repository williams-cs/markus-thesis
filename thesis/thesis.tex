...
The first experiment, \texttt{hostname.shard}, involves using verifying the \texttt{single\_command} application class works correctly on a simple program.
\begin{lstlisting}[language=Shard]
# hostname.shard
...
The \texttt{hostname.shard} program is ran a total of 100 trials on each of the three clusters.
...
The second experiment, \texttt{wordcount.shard}, verifies that the \texttt{data\_parallel} application class works correctly.
\begin{lstlisting}[language=Shard]
# hostname.shard
source cluster_setup.shard
cat input.txt |
...
The input document for this experiment is the Hamlet's Soliloquy (``To be or not to be'') from Shakespeare's Hamlet, which was chosen as an example of a famous passage with some amounts of repeating words.
...
\todoi{Evaluate whether this is a good choice for input text - maybe use this input text for the ``Reliable'' section instead; also figure out copyrights}

\todoi{Experiment 3: \texttt{wordcount\_slow.shard} (Exercise parallel tasks)}
...
\todoi{Experiment 1: \texttt{network.shard} (Try to connect to unreachable host in cluster)}
