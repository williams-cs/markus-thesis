...
% \todoi{Expand beta distribution importance}
...
The experiments in this section will be carried out multiple times, each time with one of the following two sets of remote machines:
\begin{itemize}
...
\begin{sloppypar}
  The first experiment in this section uses the Shard program \texttt{memory\_usage.shard} to verify that the \texttt{single\_command} application class works correctly on a simple program.
\end{sloppypar}
...
\begin{figure}[h]
  \begin{center}
    ...
    \caption{Number of iterations complete over time when running \texttt{memory\_usage.shard}.}
    \label{fig:memoryusage}
  \end{center}
\end{figure}

The \texttt{memory\_usage.shard} program is ran 100 times sequentially on each of the two clusters.
...
\subsection{Data parallel}
The second experiment in this section uses the program \texttt{word\_count.shard} to verify that the \texttt{data\_parallel} application class works correctly.
...
\begin{figure}[h]
  \begin{center}
    ...
    \caption{Number of iterations complete over time when running \texttt{word\_count.shard}.}
    \label{fig:wordcount}
  \end{center}
\end{figure}

The \texttt{word\_count.shard} program is ran 10 times sequentially on each of the two clusters.
...
The third experiment in this section uses the Shard program \texttt{word\_count\_slow.shard} to demonstrate Shard's ability to run many tasks in parallel on the same remote machine.
The program \texttt{word\_count\_slow.shard} is almost identical to the previous \texttt{word\_count.shard} experiment, except that each \texttt{map} remote command sleeps for 1 second in the middle before its completion.
...
\begin{figure}[h]
  \begin{center}
    ...
    \caption{Number of iterations complete over time when running \texttt{word\_count\_slow.shard}.}
...
This is especially important because there is a cap to how many simultanous commands can be ran on a single remote process, which can be quickly reached from many of the remote commands spending a large portion of its time sleeping.
...
For this section, the \texttt{word\_length.shard} program is run once again on the same input document as previously, which is Hamlet's Soliloquy (``To be or not to be'') from Shakespeare's Hamlet.
The full text of the input document and the expected output can be found in the appendix.
...
Nevertheless, Shard is able to recover from the remote Shard process unexpectedly crashing many times, which shows the resilience of the Shard distributed system to severe disruptions.
...
The programs \texttt{word\_length.shard} is run with different chances of random program failure, and the results are compared.
