...
\todoi{Finish up this section: add a picture of the shell?}
...
The parser is implemented using the parser combinator library Angstrom by following the grammar section in the POSIX shell specification, and extending it with the additional syntax found in the \textbf{Language design} section of the \textbf{Design} chapter.
...
The OCaml variant type can be used very effectively to represent recursive structures with complex definitions, along with retrieving their data in a type-safe manner using pattern matching, which is one of the significant benefits to use OCaml for implementing programming languages.

\todoi{Cite Real World Caml about variants}
...
SSH is a widely supported protocol for accessing a command line shell on a remote machine over a network.
SSH supports security, authentication, and privacy, so those features can be leveraged by Shard and do not have to be built into the Shard protocol implementation from scratch.
...
For each remote machine in the cluster, the local user should have SSH access to that machine (with an SSH key, not a password), by adding the local user's public key to the \texttt{authorized\_keys} of the remote host.
If the user has an SSH config file at the default location (at \texttt{~/.ssh/}), it is used to allow custom host names and private key names, which can be used for remote targets in the cluster definition.
Otherwise, the user should make sure that the necessary keys are added to the SSH agent for automatic authentication.
...
\todoi{Cite ssh, in particular citation should refer to features described here, and maybe also refer to ubiquity of SSH; also cite ssh config/ssh agent}
...
\todoi{Add diagram with this workflow}
...
\begin{itemize}
  \item The \texttt{-Rls} argument indicates the creation of a Local Sender.
  \item The \texttt{-Rlr} argument indicates the creation of a Local Receiver.
  \item The \texttt{-Rrs} argument indicates the creation of a Remote Sender. This also has an additional optional argument to indicate the port of the RPC server
  \item The \texttt{-Rrr} argument indicates the creation of a Remote Receiver.
\end{itemize}
\todoi{Make this a table}
...
\todoi{Issue: this section is currently inaccurate at describing how the implementation actually works, because I haven't gotten around to converting the single command application class to using the remote cluster abstraction yet}
...
\todoi{Describe remote host choosing mechanism}
...
% At some level, performance matters for the usability, but

% The sacrifice to performance is justified by the targetted use-case of Shard.
% Performance is of the highest priority in contexts where scalability matters, where

\todoi{Finish this section}
...
% Another tradeoff already mentioned, this time in the \textbf{Data transfer framework} section, is the tradeoff of using processes rather than threads for additional .
% One downside of OCaml is thatm by default, an OCaml process only supports OCaml execution on a single core.
% \todoi{Cite lack of multi core support}
\todoi{Tradeoff: threads vs processes}
