...
\begin{figure}[h]
  \begin{center}
    \includegraphics[scale=0.9]{img/experiments/e1_1620960581241.png}
...
The \texttt{word\_length.shard} experiment is run 4 times, with the crashes chance varying from 0 percent to 3 percent for each trial, but uniform for all machines in any particular trial.
...
\begin{figure}[h]
  \begin{center}
    ...
    \caption{Cumulative time taken to complete map and reduce tasks for \texttt{word\_length.shard} based on the chance for a job to randomly crash}
...
The \texttt{random\_length.shard} experiment is run 5 times, with the chance of random failure varying from 0 percent to 75 percent for each trial, but uniform for all machines in any particular trial.
Figure \ref{fig:failhash} shows the time it takes for the program to complete, based on the random failure chance of each remote machine.
...
\begin{figure}[h]
  \begin{center}
    ...
    \vspace{2cm}
    \caption{Time taken to run \texttt{random\_hash.shard} based on the chance for a job to randomly fail}
    \label{fig:failhash}
  \end{center}
\end{figure}
\todoi{Analyze the results of what this graph looks like}
...
The \texttt{word\_length.shard} experiment is run 10 times, with the chance of random failure varying from 0 percent to 50 percent for each trial, but uniform for all machines in any particular trial.
Figure \ref{fig:failhash} shows the time it takes for the program to complete, based on the random failure chance of each remote machine.
...
\begin{figure}[h]
  \begin{center}
    ...
    \caption{Cumulative time taken to complete map and reduce tasks for \texttt{word\_length.shard} based on the chance for a job to randomly fail}
...
The \texttt{random\_hash.shard} program is ran for each of 0\%, 25\%, 50\%, and 75\% packet loss rate, with the same packet loss rate being applied to all of the machines in the cluster.
...
\todoi{Re-describe rate limiting}
...
\begin{figure}[h]
  \begin{center}
    ...
    \caption{Cumulative number of trials complete given a certain amount of time passed for \texttt{line\_length.shard} versus \texttt{line\_length\_ssh.sh}}
